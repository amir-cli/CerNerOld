...
local function chat_list(msg)
    ...
    for k,v in pairsByKeys(data[tostring(groups)]) do
        ...
        local file = io.open("./CerNer/chat/lists/listed_groups.txt", "w")
        file:write(message)
        file:flush()
        file:close()
	return message
...
if to == 'user' or service or is_admin1(msg) and to == "chat" or to == "channel" then
	...
	savelog(msg.to.id, name_log.." ["..msg.from.id.."] Used /chatlist")
		if is_admin1(msg) and msg.to.type == 'chat' or msg.to.type == 'channel' then
			chat_list(msg)
			send_document("chat#id"..msg.to.id, "./CerNer/char/lists/listed_groups.txt", ok_cb, false)
			send_document("channel#id"..msg.to.id, "./CerNer/chat/lists/listed_groups.txt", ok_cb, false)
		elseif msg.to.type == 'user' then
			chat_list(msg)
			send_document("user#id"..msg.from.id, "./CerNer/chat/lists/listed_groups.txt", ok_cb, false)
		end
	end
end
